...
оплаты), «📊 Статистика» (сводка по пользователям, товарам, заявкам и
выплатам) и «⚙ Настройки» → «🏦 Банки» (просмотр полного справочника
банков, включение/отключение конкретного банка, добавление нового банка —
без хотя бы одного активного банка пользователи не могут сохранить
...
- **Раздел «⚙ Настройки»** в админ-боте реализует управление справочником
  банков («🏦 Банки»): просмотр всех банков, включение/отключение и
  добавление новых. Управление списком администраторов через интерфейс
  бота не реализовано и оставлено на усмотрение дальнейшего развития.
- **Справочник банков** заполняется миграцией `0003_seed_banks` восемью
  крупнейшими российскими банками при первом развёртывании. Если по
  какой-то причине справочник окажется пуст (например, миграции не были
  применены), администратор может добавить банки напрямую через раздел
  «⚙ Настройки» → «🏦 Банки» в боте — обращаться к базе данных вручную
  больше не требуется.
